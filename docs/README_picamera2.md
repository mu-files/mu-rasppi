# Raspberry Pi Camera: DNG Writing Benchmark

Performance comparison of DNG writing libraries for Raspberry Pi cameras, demonstrating **muraw's speed, compression, and features** compared to PiDNG.
...
muraw    uncompressed           w=1     21.6±12.2 46.19    23.57    1.0x
muraw    jpeg_lossless          w=1    639.2±3.4   1.56    16.45    1.4x
muraw    jpeg_lossless          w=2    329.1±1.4   3.04    16.45    1.4x
muraw    jpeg_lossless          w=4    175.3±1.3   5.70    16.45    1.4x
muraw    jxl_lossless           w=1    588.8±2.3   1.70     9.89    2.4x
muraw    jxl_lossless           w=2    356.6±2.4   2.80     9.89    2.4x
muraw    jxl_lossless           w=4    271.6±3.6   3.68     9.89    2.4x
muraw    jxl_lossy              w=1   1028.2±3.9   0.97     1.14   20.7x
muraw    jxl_lossy              w=2    618.6±2.8   1.62     1.14   20.7x
muraw    jxl_lossy              w=4    509.4±5.1   1.96     1.14   20.7x
muraw    uncompressed+preview   w=1    447.9±3.5   2.23    23.80    1.0x
muraw    jxl_lossless+preview   w=4    701.4±4.1   1.43    10.12    2.3x
muraw    jxl_lossy+preview      w=4    943.6±3.2   1.06     1.37   17.2x
```

**Key Findings:**
- **muraw uncompressed is 1.8× faster** than PiDNG (22ms vs 40ms)
...
- **Preview generation** adds ~430ms to render a color-corrected JPEG preview embedded in the DNG

## PiDNG vs muraw Comparison
...
- **Full-featured encode pipeline**: Tiled compression, JPEG lossless, JPEG XL lossless/lossy, multi-core
- **Embedded preview support**: JPEG preview images in DNG files for instant thumbnails
...
venv/bin/python picamera2_capture.py --mode single --preview -o capture.dng

# With compression
venv/bin/python picamera2_capture.py --mode single --compression jxl_lossless --workers 4 --preview
